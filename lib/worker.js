...
var newUserPassOpts = function(opts) {
  var newOpts = {};
  var authMechanism = opts.auth_mechanism;
  if (authMechanism === 'MONGODB') {
    if (semver.gte(opts.version, '3.0.0')) {
  ...
  case 'standalone':
    newOpts.auth = null;
    if (semver.gte(opts.version, '3.0.0')) {
      newOpts.setParameter = 'authenticationMechanisms=' + authMechanism;
    }
    return newOpts;
  case 'replicaset':
    newOpts.auth = null;
  ...
  case 'cluster':
    ...
    if (semver.gte(opts.version, '3.0.0')) {
      newOpts.replicasetOptions.setParameter = 'authenticationMechanisms=' + authMechanism;
    }
...
var createUser = function(db, username, password, roles, callback) {
  debug('Adding User');
  db.addUser(username, password, roles, function(err, result) {
    debug('Create user result: ' + JSON.stringify(result));
    callback(err, db);
  });
};

var createAdmin = function(db, username, password, authDB, callback) {
...
var setSSLOptions = function(inOpts, outOpts) {
  if (inOpts.sslMode) {
    outOpts.ssl = true;
    outOpts.sslMode = inOpts.sslMode;
  }
  if (inOpts.sslCAFile) {
    outOpts.sslCAFile = inOpts.sslCAFile;
    outOpts.ca = [fs.readFileSync(inOpts.sslCAFile, 'utf8')];
  }
  if (inOpts.sslPEMKeyFile) {
    outOpts.sslPEMKeyFile = inOpts.sslPEMKeyFile;
    outOpts.cert = fs.readFileSync(inOpts.sslPEMKeyFile, 'utf8');
    outOpts.key = fs.readFileSync(inOpts.sslPEMKeyFile, 'utf8');
  }
  if (inOpts.sslAllowConnectionsWithoutCertificates) {
    outOpts.sslAllowConnectionsWithoutCertificates = null;
  }
  if (inOpts.sslPEMKeyPassword) {
    outOpts.sslPEMKeyPassword = inOpts.sslPEMKeyPassword;
    outOpts.passphrase = inOpts.sslPEMKeyPassword;
  }
  outOpts.rejectUnauthorized = false;
};
...
var startCluster = function(opts) {
  ...
  setSSLOptions(opts, options.replicasetOptions);
  setSSLOptions(opts, options.mongosOptions);
...
module.exports = function(opts, done) {
  ...
  server.start({
    ...
    debug('Worker is done starting server without any auth');

    if (opts.auth_mechanism === 'MONGODB') {
      debug('Waiting for start...');
      setTimeout(function() {
        var url = format('mongodb://localhost:%s/%s', opts.port, opts.authDB);
        debug('User/Pass auth enabled... connecting to MongoDB at ' + url);
        mongodb.MongoClient.connect(url, function(err, db) {
          ...
          createAdmin(db, opts.username, opts.password, opts.authDB, function(err, db) {
            if (err) {
              return done(err);
            }
            ...
            db.close(function(err) {
              ...
              debug(opts.version);
              server.setCredentials('default', opts.authDB, opts.username, opts.password);
              server.restart(opts, function(err) {
                ...
                debug('Waiting for reconfiguration...');
