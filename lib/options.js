...
function Options(recipe){
  if (!(this instanceof Options)) return new Options(recipe);
  this.recipe = recipe;
  this.data = {};

  async.series(this.keys.map(function(k){
...
Options.prototype.keys = ['dbpath', 'port'];
...
Options.prototype.dbpath = function(fn){
  ...
  search.push(path.resovle(process.cwd(), '/mongodb/data/' + this.recipe.name));
