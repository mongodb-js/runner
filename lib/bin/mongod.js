...
function MongoD(opts){
  ...
  this.on('ready', this.onReady.bind(this))
    .on('lifecycle', function(name, previous){
      ...
      if(previous !== null){
        this._dumpLifecycleMessages(previous);
      }
    ...
    .on('exit', this._dumpLifecycleMessages.bind(this, 'shutdown'));
...
MongoD.prototype._dumpLifecycleMessages = function(lifecycle){
      this.debug('{"LIFECYCLE": "%s", "messages": [', lifecycle);
      this.lifecycle[lifecycle].map(function(line){
        this.debug('  "%s",', line.message);
      }.bind(this));
      this.debug('], "LIFECYCLE": "%s"}', lifecycle);
};
...
MongoD.prototype.inspectLogLine = function(line){
  ...
  // if(line.message.indexOf('\t') === 0){
  //   this.logBuffer.push(line.message.replace('\t ', ''));
  //   return;
  // }
  ...
  if(!lifecycle){
    this.debug('unknown lifecycle %s: %s', line.thread, line.message);
  }
};
