...
function ensureMongoInstalled(opts, done) {
  which('mongod', function(err, mongod) {
    if (err && err.message === 'not found: mongod') {
      debug('mongodb not installed!  installing via mongodb-version-manager');
      return mvm.use({
    ...
    debug('existing mongodb installation found at `%s`', mongod);
    return done();
  });
}
...
var exec = module.exports = exports = function(opts, done) {
  ...
  async.series([
    ensureMongoInstalled.bind(null, opts),
