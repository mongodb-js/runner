...
var start = function(opts, done) {
  ...
  killIfRunning(opts, function(err) {
    ...
    getPIDPath(opts, function(err, pidPath) {
      if (err) return done(err);
      var uri = '';
      if (opts.auth && !opts.createAdmin){
        uri = 'mongodb://adminUser:adminPass@localhost:' + opts.port;
      }
      else {
        uri = 'mongodb://localhost:' + opts.port;
      }
...
var createAdmin = function (db, callback) {
  debug('Adding admin User');
  db.admin().addUser('adminUser', 'adminPass', 
    {roles: [ { role: "root", db: "admin" } ]},
    function(err, result) {
    if (err) return callback("Create Admin Error: " + err);
    callback();
  });
}
/**
...
var stop = function (opts, done) {
  debug("stopping!");
  if(opts.auth && !opts.createAdmin){
    var url = format('mongodb://adminUser:adminPass@localhost:%s/test?authSource=admin', opts.port)
    mongodb.MongoClient.connect(url, function(err, db) {
      if (err) return done(err);
      async.series([function(callback) {
        db.admin().removeUser('adminUser',callback);
      }
      ], function(err) {
        if (err) return done(err);
...
module.exports = function(opts, done) {
  ...
  configure(opts, function(err, opts) {
    ...
    if (opts.action === 'start') {
      if (opts.auth){
        debug("start with auth");
        opts.createAdmin = true;
        start(opts, function(err, res){
            debug("stop with auth");
            if (err) return done(err);
            stop(opts, function(err, res){
              if (err) return done(err);
              opts.createAdmin = false;
