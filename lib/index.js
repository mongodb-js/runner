...
var restart = function(opts, done) {
  getPIDPath(opts, function(err, pidPath) {
    if (err) return done(err);
    getPID(opts, function(err, pid) {
      if (err) return done(err);
      ...
      debug('restarting existing pid', pid);
      try {
          ...
          if (opts.action === 'start'){
            ...
            return start(opts, function(err, res){
             ...
             function(callback) {
              ...
              function(callback) {
                var url = format('mongodb://localhost:%s/test',
                                 opts.port);
                mongodb.MongoClient.connect(url, callback);
              },
              function(db, callback) {
              ...
              function(callback) {
                ...
                debug('done starting cluster with user/pass enabled');
                return done(err);
              });
            });
          }
