...
function connect(url, opts, done) {
  var call = backoff.call(mongodb.MongoClient.connect, url, opts, function(err, db) {
    if (err) {
      console.log('%s connection attempts to %s failed.  giving up.', url, call.getNumRetries());
      return done(err);
    }
    console.log('Successfully connected to %s after %s attempts!', url, call.getNumRetries());
    done(null, db);
  });
  ...
  call.on('backoff', function(number, delay) {
    console.log('connect attempt #%s failed.  retrying in %sms...', number, delay);
  });
  call.failAfter(25);
  call.start();
}
...
var buildURL = function(opts, dbName) {
  var urlObj = {
    protocol: 'mongodb',
    slashes: true,
    pathname: dbName,
    ...
    getPIDPath(opts, function(err, pidPath) {
      ...
      fs.writeFile(pidPath, proc.pid, function(err) {
        ...
        async.waterfall([function(callback) {
          var uri = buildURL(opts, 'admin');
          debug('URI is %s', uri);

          debug('waiting for a successful connection to `%s`...', uri);
          connect(uri, connectionOptions, callback);
        }, function(db, callback) {
          db.close(callback);
        }, function(result, callback) {
          setTimeout(function() {
            if (opts.secondUser) {
              var uri = buildURL(opts, opts.secondUser.db);
              debug('URI is %s', uri);

              debug('waiting for second successful connection to `%s`...', uri);
              connect(uri, connectionOptions, callback);
            } else {
          ...
          if (opts.thirdUser) {
            var uri = buildURL(opts, opts.thirdUser.db);
            debug('URI is %s', uri);

            debug('waiting for third successful connection to `%s`...', uri);
            connect(uri, connectionOptions, callback);
          } else {
            return done(null, opts);
          }
